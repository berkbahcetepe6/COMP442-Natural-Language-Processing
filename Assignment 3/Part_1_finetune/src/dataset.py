...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 2:
        prefix_len = random.randint(1, truncated_len-2)
        
        average_masked_len = int(truncated_len*(1/4))
        variance = min(average_masked_len-1, truncated_len-2)
        masked_len = random.randint(average_masked_len-variance,average_masked_len+variance)
        ...
        # 3:
        x_ = prefix + self.MASK_CHAR + suffix  + self.MASK_CHAR + masked_content
        x_ = x_ + self.PAD_CHAR * (self.block_size - len(x_))
