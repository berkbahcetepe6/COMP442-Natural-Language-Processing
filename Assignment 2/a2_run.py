...
def train(opts: argparse.Namespace):
    ...
    turkish_word2id = a2_dataloader.read_word2id_from_file(opts.turkish_vocab)
    ...
    train_dataloader = a2_dataloader.wmt16DataLoader(
        opts.training_dir, turkish_word2id, english_word2id, opts.source_lang,
    ...
    dev_dataloader = a2_dataloader.wmt16DataLoader(
        opts.training_dir, turkish_word2id, english_word2id, opts.source_lang,
        dev_prefixes, batch_size=opts.batch_size,
        pin_memory=(opts.device.type == 'cuda'),
        num_workers=1,
    )
    del dev_prefixes, turkish_word2id, english_word2id
...
def test(opts: argparse.Namespace):
    ...
    english_word2id = a2_dataloader.read_word2id_from_file(opts.english_vocab)
    dataloader = a2_dataloader.wmt16DataLoader(
        opts.testing_dir, turkish_word2id, english_word2id, opts.source_lang,
        batch_size=opts.batch_size,
        pin_memory=(opts.device.type == 'cuda')
    )
    del turkish_word2id, english_word2id
...
def build_training_parser(
    ...
    parser.add_argument(
        'turkish_vocab', type=possible_gzipped_file,
        help='Turkish vocabulary file'
    )
    parser.add_argument(
...
class readable_dir(argparse.Action):
    ...
    def __call__(self,
        ...
        if not isinstance(values, str):
            raise ValueError(f"Expected values is str but found {values}")
